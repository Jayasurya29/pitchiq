...
def approve_email(db: Session, research_id: int):
    """Approve email only"""
    record = db.query(ResearchHistory).filter(
        ResearchHistory.id == research_id
    ).first()
    if record:
        ...
        if record.linkedin_approval_status == "approved":
            record.approval_status = "approved"
        db.commit()
        db.refresh(record)
        print(f"✅ Email approved for ID {research_id}")
    return record


def approve_linkedin(db: Session, research_id: int):
    ...
    if record:
        ...
        if record.email_approval_status == "approved":
            record.approval_status = "approved"
        db.commit()
        db.refresh(record)
        print(f"✅ LinkedIn approved for ID {research_id}")
    return record


def reject_email(db: Session, research_id: int):
    """Reject email only"""
    record = db.query(ResearchHistory).filter(
        ResearchHistory.id == research_id
    ).first()
    if record:
        ...
        db.commit()
        db.refresh(record)
        print(f"❌ Email rejected for ID {research_id}")
    return record


def reject_linkedin(db: Session, research_id: int):
    ...
    if record:
        ...
        db.commit()
        db.refresh(record)
        print(f"❌ LinkedIn rejected for ID {research_id}")
    return record